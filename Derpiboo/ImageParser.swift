...
struct ImageParser: ParserForItem {
    ...
    static func parse(dictionary item: NSDictionary) -> Promise<ImageResult> {
        guard let id = item["id"] as? String else { return Promise(error: ParserError.parserGuardFailed(id: "0", variable: "id")) }
        ...
        guard let duplicate_reports = item["duplicate_reports"] as? [AnyObject] else { return Promise(error: ParserError.parserGuardFailed(id: id, variable: "duplicate_reports")) }
        ...
        guard let tag_ids = item["tag_ids"] as? [String] else { return Promise(error: ParserError.parserGuardFailed(id: id, variable: "tag_ids")) }
